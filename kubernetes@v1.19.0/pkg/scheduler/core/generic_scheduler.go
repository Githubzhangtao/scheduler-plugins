...
import (
	...
	"fmt"
	"math/rand"
	"sigs.k8s.io/scheduler-plugins/pkg/concurrentscheduling"
	"sort"
	"strings"
	"sync"
	"sync/atomic"
...
func (g *genericScheduler) Schedule(ctx context.Context, prof *profile.Profile, state *framework.CycleState, pod *v1.Pod) (result ScheduleResult, err error) {
	...
	klog.V(1).Infof("%v-predicate-%v", pod.Name, metrics.SinceInSeconds(startPredicateEvalTime)*1000)
	...
	//fmt.Println("----------------------------------------------------打分------------------------------------------------------------------------")
	klog.V(2).Infof("排序前：%v", priorityList)
	concurrentInstance := concurrentscheduling.GetConcurrentScheduler() // 修改
	...
	for _, s := range scoreList {
		...
		//klog.V(1).Infof("长度+1:%v",concurrentInstance.Queue.Len())
	}
	//klog.V(2).Infof("队列长度:%v",concurrentInstance.Queue.Len())
	thePodHost, _ := concurrentInstance.Queue.Get()
	//klog.V(1).Infof("队列长度:%v",concurrentInstance.Queue.Len())

	host := thePodHost.(framework.NodeScore).Name
	klog.V(2).Infof("thePod选择的节点为%v", host)
	//klog.V(2).Infof("其余Pod可用的得分队列:%v,长度为:%v", concurrentInstance.Queue, concurrentInstance.Queue.Len())
	...
	klog.V(1).Infof("%v-priority-%v", pod.Name, metrics.SinceInSeconds(startPriorityEvalTime)*1000)
...
func (g *genericScheduler) findNodesThatFitPod(ctx context.Context, prof *profile.Profile, state *framework.CycleState, pod *v1.Pod) ([]*v1.Node, framework.NodeToStatusMap, error) {
	...
	// V1版本不需要以下代码，注释掉。V2版本需要以下代码，关闭注释。
	//klog.V(2).Info("----------------------------------------------------预选侵入------------------------------------------------------------------------")
	/*concurrentInstance := concurrentscheduling.GetConcurrentScheduler()
	var feasibleNodes []*v1.Node
	host := concurrentInstance.PodToNode[pod.Name]
	if host != "" {
		klog.V(2).Infof("当前Pod:%v是NormalPod，跳过预选",pod.Name)
		allNodes, _ := g.nodeInfoSnapshot.NodeInfos().Get(concurrentInstance.PodToNode[pod.Name])
		feasibleNodes = append(feasibleNodes, allNodes.Node())
		filteredNodesStatuses[allNodes.Node().Name] = framework.NewStatus(0,"")
		return feasibleNodes, filteredNodesStatuses, nil
	}*/
	//klog.V(2).Info("----------------------------------------------------预选侵入结束------------------------------------------------------------------------")
	//klog.V(2).Infof("当前Pod:%v是ThePod，执行预选流程",pod.Name)
